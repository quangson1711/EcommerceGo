...
func (s *Store) CreateOrder(order *types.Order) error {
	result, err := s.db.Exec("INSERT INTO orders (userId, total, status, address, createAt) VALUES (?,?,?,?,?)", order.UserID, order.Total, order.Status, order.Address, order.CreatedAt)
	if err != nil {
		return err
	}
